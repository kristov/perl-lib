...
is( $p2->X + 0, 5, 'X accessor' );
is( $p1->Y + 0, 0, 'Y accessor' );
is( $p1->distance( $p2 ) + 0, 5, 'distance()' );
is( $p1->distance( $p1 ) + 0, 0, 'distance() from self' );
is( $p2->distance( $p1 ) + 0, 5, 'distance() in negative' );

my $p3 = $p2->point_angle_distance_from( NINTEY_DEG_IN_RAD, 10 );
is( sprintf( '%0.0f', $p3->X ) + 0, 5, 'point_angle_distance_from(): X correct' );
is( sprintf( '%0.0f', $p3->Y ) + 0, 10, 'point_angle_distance_from(): Y correct' );

my $pc = $p3->closest_of_two( $p1, $p2 );
is( $pc->X + 0, 5, 'closest_of_two(): X correct' );
is( $pc->Y + 0, 0, 'closest_of_two(): Y correct' );
